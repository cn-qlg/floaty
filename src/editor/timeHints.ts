...
function matchRelativeMinutes(text: string, now: Date): TimeHint | null {
  const m = text.match(/(\d+)\s*(分钟|分|min|m)\s*后?\s*$/i);
  if (!m) return null;
...
function matchRelativeHours(text: string, now: Date): TimeHint | null {
  const m = text.match(/(\d+)\s*(小时|时|hour|hr|h)\s*后?\s*$/i);
  if (!m) return null;
...
function matchRelativeDays(text: string, now: Date): TimeHint | null {
  const m = text.match(/(\d+)\s*(天|day|d)\s*后?\s*$/i);
  if (!m) return null;
...
function matchTomorrowWithTime(text: string, now: Date): TimeHint | null {
  // 明天 / 明 天 上午 9 点 / 明天 9:00 / tomorrow 9am — 容忍输入法插入的空格
  const m =
    text.match(/明\s*天\s*(上午|下午|早上|晚上)?\s*(\d{1,2})\s*[点:]?\s*(\d{1,2})?\s*$/) ||
    text.match(/tomorrow\s+(\d{1,2})\s*(?::(\d{2}))?\s*(am|pm)?\s*$/i);
  if (!m) return null;
  const d = new Date(now);
...
function matchHouAfterDays(text: string, now: Date): TimeHint | null {
  const m = text.match(/(大\s*后\s*天|后\s*天)\s*$/);
  if (!m) return null;
  const d = new Date(now);
...
function matchNextWeekday(text: string, now: Date): TimeHint | null {
  // 周三 / 下周三 / 下 周 三 — 容忍空格
  const m = text.match(/(下)?\s*周\s*([一二三四五六日天])\s*$/);
  if (!m) return null;
...
function matchAbsoluteMonthDay(text: string, now: Date): TimeHint | null {
  const m = text.match(/(\d{1,2})\s*月\s*(\d{1,2})\s*日?\s*$/);
  if (!m) return null;
...
function matchAbsoluteISO(text: string, _now: Date): TimeHint | null {
  const m = text.match(/(\d{4})\s*-\s*(\d{1,2})\s*-\s*(\d{1,2})\s*$/);
  if (!m) return null;
